...
func insertBitmexTrade(timestamp, symbol, side string, size, price float64, tickDirection, trdMatchID string, grossValue, homeNotional, foreignNotional float64) error {
	sql := "insert into test.bitmextrade(timestamp,symbol,side,size,price,tickDirection,trdMatchID,grossValue,homeNotional,foreignNotional) values(?,?,?,?,?,?,?,?,?,?)"
	_, err := db.Exec(sql, timestamp, symbol, side, size, price, tickDirection, trdMatchID, grossValue, homeNotional, foreignNotional)
	if err != nil {
		fmt.Println(err.Error())
		return err
	}

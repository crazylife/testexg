...
func main() {
	...
	var status_monitor_addr, service_name, service_ip, consul_addr, found_service string
	var service_port int
	flag.StringVar(&consul_addr, "consul_addr", "alihk-debug-3.qbtrade.org:8500", "host:port of the service stuats monitor interface")
	flag.StringVar(&status_monitor_addr, "monitor_addr", "alihk-debug-3.qbtrade.org:8500", "host:port of the service stuats monitor interface")
	flag.StringVar(&service_name, "service_name", "HelloService", "name of the service")
	flag.StringVar(&service_ip, "ip", addr, "service serve ip")
	flag.StringVar(&found_service, "found_service", "HelloService", "found the target service")
	flag.IntVar(&service_port, "port", 50051, "service serve port")
	flag.Parse()
	...
	case "reg":
		...
		break
	case "get":
		DoDiscover(consul_addr, found_service)
		break
	case "unreg":
		WaitToUnRegistService()
		break
	}
...
func DoRegistService(consul_addr string, monitor_addr string, service_name string, ip string, port int) {
	...
	var tags []string
	testStr := fmt.Sprintf("%v:%v/%v", ip, port, service_name)
	fmt.Println(testStr)
	service := &api.AgentServiceRegistration{
		...
		Check: &api.AgentServiceCheck{
			Interval: "10s",
			GRPC:     fmt.Sprintf("%v:%v/%v", ip, port, service_name),
			Timeout:  "5s",
		},
	}
